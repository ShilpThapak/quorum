...
//
// Reference (correct) solution: guard every access to the cache map
// and the LRU list with a sync.Mutex, so the `-race` detector is
// happy even though the mock server hammers the cache from many
// goroutines at once.
//
